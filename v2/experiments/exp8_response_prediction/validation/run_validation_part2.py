#!/usr/bin/env python3
"""
Task 4 Part 2: LODO-CV permutation test (reduced) + Calibration curves.
Parts A and B1 already completed. This script does B2 and C.
"""
...
# =====================================================================
# PART B2: LODO-CV Permutation Test (reduced to 200 permutations)
# =====================================================================
print("\n" + "=" * 70, flush=True)
print("PART B2: Overall LODO-CV Permutation Test (200 permutations)", flush=True)
print("=" * 70, flush=True)
...
for i in range(N_PERM_LODO):
    ...
    # Permute response labels WITHIN each dataset
    y_perm = y_all.copy()
    for ds in unique_datasets:
        ds_indices = np.where(datasets_all == ds)[0]
...
ax.hist(perm_mean_aucs, bins=30, edgecolor='black', alpha=0.7, density=True)
...
print(f"  I-SPY2 interaction (all): permutation p={perm_results.get('interaction_permutation', {}).get('all_ispy2', {}).get('permutation_p_onesided', 'N/A')}", flush=True)
print(f"  I-SPY2 interaction (GSE194040): permutation p={perm_results.get('interaction_permutation', {}).get('gse194040', {}).get('permutation_p_onesided', 'N/A')}", flush=True)
print(f"  LODO-CV overall: permutation p={perm_p_lodo:.4f}", flush=True)
